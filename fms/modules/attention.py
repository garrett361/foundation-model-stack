        ...
        **attn_kwargs: Unpack[AttentionKwargs],
        ...
        if attn_compute_dict["is_prefill"](**updated_attn_kwargs):
            attn = attn_compute_dict["compute_prefill"](
                ...
                self.scale_factor,
                # TODO: `_math_attention_with_sinks_op` ("sdpa_with_sinks") does not accept
                # `group` and will raise TypeError if CP and sinks are used together. All three
                # registered compute ops that reach this call site have inconsistent signatures:
                #   - _sdpa_compute_op        ("sdpa_causal")     -- has `group` positional arg
                #     (added by the cp-clean branch before the rebase)
                #   - _sdpa_compute_op_cp     ("sdpa_causal_cp")  -- has `group` positional arg
                #   - _math_attention_with_sinks_op ("sdpa_with_sinks") -- no `group` param;
                #     added by upstream after this branch diverged (commit 32f55a72 era)
                # Fix: add `group: Optional[ProcessGroup] = None` to
                # `_math_attention_with_sinks_op` so all ops share the same positional
                # calling convention. For now, CP + sinks together is untested/broken.
                self.group,
                **updated_attn_kwargs,
            )
            #attn_nan = torch.isnan(attn).any()
