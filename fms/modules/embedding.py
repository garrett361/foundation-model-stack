...
# TODO: upstream removed WordEmbedding/TPWordEmbedding (commit 32f55a72). Follow the
# upstream embedding patterns and drop these classes once CPWordEmbedding/CPEmbedding
...
class CPWordEmbedding(WordEmbedding, CPModule):
    """
    Input/output embedding layer for sequence models.
    Includes vocabulary and optional absolute positional encodings.
    Can optionally include output embeddings, to provide "reversed" output prediction logits.
    ...
    world_size: int
        the number of processes running this model in CP
    rank: int
        the index of this process wrt to the rest running the model in CP
    """
    ...
    @staticmethod
    def import_module(we: WordEmbedding, group: ProcessGroup) -> "CPWordEmbedding":
        cp_we = CPWordEmbedding(
            vocab_size=we.vocab_size,
            emb_dim=we.emb_dim,
            padding_idx=we.padding_idx,
            max_pos=we.max_pos,
...
class CPEmbedding(nn.Embedding, CPModule):
    """
    Input embedding layer for sequence models. Not to be confused with CPWordEmbedding.
    (CP)WordEmbedding supports fusing together the LM Head and the input Embedding, while
    this is a class for when you want them separate, like in headless models.
    ...
    @staticmethod
    def import_module(e: nn.Embedding, group: ProcessGroup) -> "CPEmbedding":
        cp_e = CPEmbedding(
            num_embeddings=e.num_embeddings,
            embedding_dim=e.embedding_dim,
            padding_idx=e.padding_idx,
