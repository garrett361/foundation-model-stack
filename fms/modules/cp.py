...
class CPModule(nn.Module, metaclass=ABCMeta):
    ...
    Context Parallel. On top of inheriting from this class, the CP module
    will have to implement 
    ....list_colwise_weights, list_rowwise_weights,
    ...
    def setup_cp(self, rank: int, group: Optional[dist.ProcessGroup]) -> None:
        self.rank = rank
    ...
    def sharded_copy(
        ...
        if shard_type == ShardType.SHARD:
            ...
            cp_slices = self.__get_cp_slices(
                tensor_value.shape[dim], output_size_per_partition, max_partition_sizes
            )
            cp_shard_indices = [
                tuple([slice(None) for _ in range(dim)] + [cp_slice])
                for cp_slice in cp_slices
            ]
            tensors_to_cat = [
                tensor_value[cp_shard_index] for cp_shard_index in cp_shard_indices
            ]
